...
public:
	disk(const point3& Q, const vec3& u, const vec3& v, double r, shared_ptr<material> mat)
		: Q(Q), u(u), v(v), r(r), mat(mat)
	{
		auto n = cross(u, v);
	...
	virtual bool is_interior(double a, double b, hit_record& rec) const {
		interval lt_radius = interval(-infinity, r);
		double location = std::sqrt(a * a + b * b);

		if (!lt_radius.contains(location)) return false;
